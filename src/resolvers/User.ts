...
export class UsersResolver {
    ...
    // Validate account
    @Mutation(() => String)
    async validateAccount(@Arg('validAccountToken') validAccountToken: string): Promise<string> {
  ...
  async deleteUser(@Arg("id", () => ID) id: number): Promise<Boolean> {
    ...
    // Non-existing user
    @Mutation(() => User)
    async signup(@Arg('pseudo') pseudo: string, @Arg('email') email: string, @Arg('password') password: string, @Arg('validAccountToken') validAccountToken: string): Promise<User | Boolean> {
    ...
    async signin(@Arg('pseudo') pseudo: string, @Arg('email') email: string, @Arg('password') password: string, @Arg('validAccountToken') validAccountToken: string): Promise<User | null> {
        const user = await this.userRepo.findOne({ email });
